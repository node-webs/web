...
const view = {
    page: (req: Request, res: Response) => {
        res.render('index', { title: 'PET 홈페이지' });
    },
    page4040: (req: Request, res: Response) => {
        const pageName = decodeURIComponent(req.params.page as string);
        ...
        res.send(`요청된 페이지는 없습니다. : ${pageName}`);
    }
};
